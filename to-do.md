...
- [ ] 메인 애플리케이션 창 (`QMainWindow`) 기본 UI 틀 구현 [PRD 3.4]
    ...
    - [ ] 주요 화면 영역을 위한 중앙 위젯 레이아웃 설정 (예: `QStackedWidget` 또는 유사 구조로 화면 전환 준비)
...
- [ ] **UI:** 메인 창에 "거래내역 파일 불러오기" 버튼 (`QPushButton`) 추가 [PRD 2.1]
- [ ] **UI:** 선택된 파일 경로를 표시할 레이블 (`QLabel`) 추가 [PRD 2.1]
- [ ] **로직:** 버튼 클릭 시, 운영체제 파일 탐색기(CSV 필터 적용)를 열어 파일 경로를 가져오는 함수 구현 (`app/core/file_handler.py` 등에) [PRD 2.1]
- [ ] **연결:** 가져온 파일 경로를 UI 레이블에 표시하는 로직 연결
- [ ] **테스트:** 수동으로 CSV 파일 선택 시 경로가 잘 표시되는지, 취소 시 적절히 처리되는지 확인
- [ ] **커밋:** "Feat: Implement CSV file selection and display path (Slice 1.1)"

### 슬라이스 1.2: 선택된 CSV 파일 내용(첫 5행) 파싱 및 콘솔 출력

- **🎯 목표:** 슬라이스 1.1에서 선택된 CSV 파일의 내용을 읽어, 첫 5행(헤더 포함)을 파싱하여 콘솔에 출력한다.
- [ ] **로직:** CSV 파일 경로를 입력받아, 내용을 읽고 파싱하는 함수 구현 (`app/core/file_parser.py`) [PRD 2.1]
    - [ ] 표준 CSV 모듈 사용, UTF-8 인코딩 기본 처리
    - [ ] 헤더 행과 데이터 행 구분
    - [ ] 첫 5행 데이터만 추출 (또는 지정된 수만큼)
- [ ] **연결:** 파일 선택 완료 후, 위 파싱 함수 호출 및 결과 콘솔 출력
- [ ] **테스트:** 다양한 CSV 파일(헤더 유무, 데이터 적은/많은 경우)로 콘솔 출력 확인
- [ ] **커밋:** "Feat: Parse first 5 rows of selected CSV and print to console (Slice 1.2)"
...
- [ ] **로직:** 파일 선택 후, 파일 크기 검증 로직 추가 (50MB 초과 시 오류) [PRD 2.1, 4.1]
- [ ] **로직:** 지원하지 않는 파일 확장자 선택 시 오류 메시지 처리 [PRD 4.1]
- [ ] **로직:** CSV/Excel 파싱 중 발생하는 주요 예외(형식 오류, 컬럼 없음 등) 처리 로직 보강 [PRD 4.1]
- [ ] **UI:** 오류 발생 시 사용자에게 `QMessageBox` 등으로 명확한 알림 표시
- [ ] **테스트:** 다양한 오류 상황 재현 및 메시지 확인
...
- [ ] **DB:** `categories` 테이블에서 기본 카테고리 목록("계좌 간 이체 (분석 제외)" 포함)을 가져오는 함수 구현 (`app/db/crud.py` 등) [PRD 3.2.5]
- [ ] **UI:** `QTableWidget`의 특정 열(예: "사용자 확정 카테고리")에 `QComboBox`를 각 행마다 추가
- [ ] **로직:** DB에서 가져온 카테고리 목록을 각 `QComboBox`에 채워 넣기
- [ ] **테스트:** 모든 행에 드롭다운이 잘 생성되고 카테고리 목록이 올바르게 표시되는지 확인
...
- [ ] **로직:** 현재 작업 중인 파일 정보, 각 거래별 확정 카테고리(아직 DB에 최종 저장 전이라도), 현재 작업 위치 등을 저장할 데이터 구조 정의
- [ ] **UI:** "중간 저장" 버튼 추가
- [ ] **로직:** "중간 저장" 시, 위 데이터를 특정 파일(예: JSON) 또는 `settings` 테이블에 저장
- [ ] **로직:** 프로그램 시작 시 또는 파일 재로드 시, 저장된 작업 내용이 있으면 불러와서 UI에 복원하는 기능 (0단계 기본 환경 설정과 연계)
- [ ] **테스트:** 중간 저장 후 프로그램 종료/재시작 시 작업 내용 복원 확인
- [ ] **커밋:** "Feat: Implement 'Save Progress' for classification (Slice 2.5)"
